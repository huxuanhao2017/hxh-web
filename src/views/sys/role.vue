<template>
  <div style='margin: 20px'>
    ...
    <Modal :mask-closable='false' :visible.sync='newModal' :loading='loading' v-model='newModal' width='600' title='新建'
           @on-ok="newOk('roleNew')" @on-cancel="cancel()">
    <Form ref='roleNew' :model='roleNew' :rules='ruleNew' :label-width='80'>
      ...
      <Form-item label='描述:' prop='describe'>
        <Input v-model='roleNew.describe' type='textarea' :autosize='{minRows: 2,maxRows: 5}'/>
      </Form-item>
    </Form>
    </Modal>
    <!--修改modal-->
    <Modal :mask-closable='false' :visible.sync='modifyModal' :loading='loading' v-model='modifyModal' width='600'
           title='修改' @on-ok="modifyOk('roleModify')" @on-cancel="cancel()">
    <Form ref='roleModify' :model='roleModify' :rules='ruleModify' :label-width='80'>
      <Form-item label='角色名:' prop='name'>
        <Input v-model='roleModify.name' style='width: 204px'/>
      </Form-item>
      <Form-item label='描述:' prop='describe'>
        <Input v-model='roleModify.describe' type='textarea' :autosize='{minRows: 2,maxRows: 5}'/>
      </Form-item>
    </Form>
    </Modal>
    <!-- 配置权限 -->
    <Modal v-model='settingModal' width='400' title='配置权限' @on-ok='settingOk()' @on-cancel='cancel()'
           :mask-closable='false'>
      <Row>
        <Col span='24'>
...
<script>export default {
          ...
          render: (h, params) => {
            return h('div', [
              h('i-switch', {
                ...
                on: {
                  'on-change': (val) => {
                    let i = this.moduleArr.indexOf(params.row.id + '')
                    if (val) {
                      if (i === -1) {
  ...
  methods: {
    ...
    setting(e) {
      ...
      let data2Temp = []
      for (let i = 0; i < this.submenusList.length; i++) {
